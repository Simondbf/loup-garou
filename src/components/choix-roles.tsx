...
import { ajoutPossible, ajusterRole, cartesComedienPossibles } from "@/data/composition";
...
export function ChoixRoles({
  ...
  /** Nombre de cartes que réclame une composition donnée : grise le « + » quand elle est pleine. */
          ...
          <div key={camp} className="mb-5">
            ...
            <ul className="flex flex-col gap-2">
              {duCamp.map((role) => {
                ...
                const auMax = !ajoutPossible(selection, role.id, attendu);
                return (
...
/**
 * Les trois cartes du Comédien, choisies avec le reste de la composition.
 *
 * Elles viennent de la boîte et ne comptent pas dans le total : on ne peut
 * donc pas les « ajouter » à la composition, il faut les désigner ici. Sans
 * elles, la distribution refuse de partir.
 */
export function ChoixCartesComedien({
  selection,
  cartes,
  onCartes,
}: {
  ...
  return (
    <section className="mb-4 rounded-xl border border-border p-3">
      ...
      <p className="mt-1 text-[11px] text-muted-foreground">
        Elles viennent de la boîte et ne comptent pas dans votre composition : trois personnages du
        village à pouvoir, qu'aucun joueur ne tient déjà. Elles sont posées face visible une fois
        les cartes distribuées. {cartes.length} sur 3.
      </p>
      <div className="mt-2 flex flex-wrap gap-1.5">
        {cartesComedienPossibles(selection).map((r) => {
          const prise = cartes.includes(r.id);
          return (
            <button
              key={r.id}
              onClick={() =>
                onCartes(prise ? cartes.filter((x) => x !== r.id) : [...cartes, r.id].slice(-3))
              }
              className={cn(
                "rounded-lg border px-2 py-1 text-[11px]",
                prise ? "border-primary bg-primary/15 text-primary" : "border-border bg-secondary",
              )}
            >
              {r.name}
            </button>
