...
export function ConduiteNuit({
  ...
  const etapes = useMemo(
    () => construire(game, { onAction, onLovers, onBaillon }),
    [game, onAction, onLovers, onBaillon],
  );
  ...
  /**
   * Qui répond à l'appel de ce rôle. Le Comédien pour la carte qu'il a
   * empruntée, et le Voleur pour celle qu'il a prise au centre la première
   * nuit : leur carte n'a pas encore changé de mains, mais ce sont bien eux
   * qu'on réveille.
   */
  const porteursAffiches = (roleId: string) => {
    const liste = game.seats.filter((s) => s.roleId === roleId);
    const ajouter = (carte: string) => {
      const siege = game.seats.find((s) => s.roleId === carte && s.alive);
      if (siege && !liste.includes(siege)) liste.push(siege);
    };
    if (nuit.comedien === roleId) ajouter("comedien");
    if (game.night <= 1 && game.thiefVariant === "centre" && nuit.voleurCarte === roleId) {
      ajouter("voleur");
    }
    return liste;
  };
          ...
          <div className="min-w-0 flex-1">
            ...
            {!etape.titre && (
              <p className="truncate text-[11px] text-muted-foreground">
                {porteursAffiches(etape.role.id)
...
function construire(
  ...
  const { onAction, onLovers, onBaillon } = actions;
  const nuit = game.nuit ?? {};
  const etat = game.hostState ?? {};
  const vivants = game.seats.filter((s) => s.alive);
  const premiere = game.night <= 1;
  /**
   * Le Voleur de la variante « cartes au centre » choisit la première nuit, en
   * tout premier, et joue aussitôt le rôle pris : s'il prend un Loup-Garou,
   * il se réveille avec la meute cette nuit-là. Mais rien n'est appliqué
   * avant le lever du jour — la carte ne change de mains qu'à ce moment — ce
   * qui laisse le Maître du Jeu revenir en arrière sans rien défaire.
   */
  const voleurSeat = vivants.find((s) => s.roleId === "voleur");
  const voleurPrend = premiere && game.thiefVariant === "centre" ? nuit.voleurCarte : undefined;
  const roleCetteNuit = (s: SeatDTO) =>
    voleurPrend && voleurSeat && s.position === voleurSeat.position ? voleurPrend : s.roleId;

  const enJeu = new Set(vivants.map(roleCetteNuit).filter(Boolean) as string[]);
  /**
  ...
  const porteur = (id: string) =>
    vivants.find((s) => roleCetteNuit(s) === id) ?? (empruntee === id ? comedienSeat : undefined);
  ...
  const estLoup = (s: SeatDTO) => {
    const id = roleCetteNuit(s);
    const r = id ? ROLES_BY_ID[id] : undefined;
  ...
  /* ---- Comédien ---- */
  // Trois cartes, une par nuit : une fois la dernière jouée, il n'a plus rien
  // à choisir et redevient un simple villageois. Le réveiller bloquerait la
  // nuit sur un écran sans aucune carte à toucher.
  const cartesComedien = game.comedienCartes ?? [];
  if (enJeu.has("comedien") && cartesComedien.length > 0) {
    const cartes = cartesComedien;
    e.push({
      id: "comedien",
      role: R("comedien"),
          ...
          <>
            <Profils
              joueurs={vivants.filter(
                (x) =>
                  x.position !== sorciere.position &&
                  x.position !== nuit.soin &&
                  // La victime des Loups est déjà morte : l'empoisonner ne
                  // servirait à rien. Sauf si elle a été infectée — elle
                  // survit alors, et redevient une cible comme une autre.
                  (x.position !== nuit.victimeLoups || nuit.infection === true) &&
                  x.position !== nuit.secondeVictime,
              )}
  ...
  if (flutiste) {
    ...
    e.push({
      ...
      consigne:
        "Touchez les deux nouveaux envoûtés. Il se rendort ; vous réveillerez ensuite tous les envoûtés pour qu'ils se reconnaissent.",
    ...
    // Les envoûtés l'apprennent cette nuit, juste après que le Flûtiste s'est
    // rendormi — pas au matin. Tous se réveillent, anciens et nouveaux, et se
    // reconnaissent sans savoir qui joue de la flûte.
    const tous = [...deja, ...(nuit.charmes ?? [])].filter((p) =>
      vivants.some((s) => s.position === p),
    );
    if (tous.length > 0) {
      e.push({
        id: "envoutes",
        role: R("joueur-de-flute"),
        titre: "Les envoûtés",
        appel: "« Les envoûtés se réveillent et se reconnaissent. »",
        consigne:
          "Tapez sur la tête de chacun d'eux. Qu'ils ouvrent les yeux, se regardent en silence, puis se rendorment.",
        aide: "Ils savent désormais qu'ils sont sous le charme, mais pas qui les a envoûtés. Le Joueur de Flûte gagne seul dès que tous les survivants le sont.",
        pret: ok,
        rendu: () => (
          <ul className="flex flex-col gap-2">
            {tous.map((p) => (
              <li
                key={p}
                className="rounded-xl border border-border bg-secondary p-3 text-sm font-semibold"
              >
                {nom(p)}
                {(nuit.charmes ?? []).includes(p) && (
                  <span className="text-[11px] font-normal text-primary"> — cette nuit</span>
                )}
              </li>
            ))}
          </ul>
  ...
  /* ---- Voleur ---- */
  // Il se retrouve par sa carte en base, pas par son rôle de la nuit : une
  // fois qu'il a pris une carte au centre, il en joue le pouvoir, mais c'est
  // toujours lui qu'on appelle ici.
  if (voleurSeat && (premiere || game.thiefVariant === "echange")) {
    const echange = game.thiefVariant === "echange";
    const etapeVoleur: Etape = {
      id: "voleur",
      role: R("voleur"),
      appel: echange
        ? "« Le Voleur se réveille, en tout dernier. »"
        : "« Le Voleur se réveille, en tout premier. »",
      consigne: echange
        ? "Touchez le joueur avec qui il échange sa carte, puis validez. L'échange n'aura lieu qu'au lever du jour : vous pouvez revenir en arrière sans rien défaire."
        : "Lisez-lui les deux cartes du centre, numérotées, et touchez celle qu'il prend. Il en joue le pouvoir dès cette nuit : c'est lui que vous appellerez au tour du personnage choisi.",
      aide: echange
        ? "Il vole une identité en toute fin de nuit, quand tous les autres ont déjà agi. Les cartes changent de main, les morts restent aux personnes : s'il vole un joueur désigné cette nuit, ce joueur meurt quand même, mais sous la carte du Voleur, et le Voleur survit avec le rôle volé. S'il vole un Enfant Sauvage déjà passé côté Loups, ou un Chien-Loup qui a choisi la meute, il en hérite. Chacun revérifie sa carte au réveil."
        : "Règle du livret : il choisit la première nuit, avant tout le monde, et garde ce rôle pour la partie. Sa carte de Voleur retourne au centre.",
      pret: () => (echange ? nuit.voleurEchange !== undefined : nuit.voleurCarte !== undefined),
      rendu: () =>
        echange ? (
          <Profils
            joueurs={vivants.filter((s) => s.position !== voleurSeat.position)}
            choisis={nuit.voleurEchange !== undefined ? [nuit.voleurEchange] : []}
            sur={(p) => onAction({ voleurEchange: nuit.voleurEchange === p ? null : p })}
            nom={nom}
            roleDe={roleDe}
          />
        ) : (
            ...
            <div className="flex flex-col gap-2">
              {game.centerCards.map((id, i) => {
                ...
                return (
                  <button
                    key={`${id}-${i}`}
                    onClick={() => onAction({ voleurCarte: pris ? null : id })}
                    className={cn(
                      "flex items-center gap-3 rounded-xl border p-3 text-left",
                      pris
                        ? "border-primary bg-primary/15 text-primary"
    ...
    // Variante au centre : en tout premier, la première nuit. Échange : en
    // tout dernier, chaque nuit.
    if (echange) e.push(etapeVoleur);
    else e.unshift(etapeVoleur);
  }
  ...
  const denouement = denouementNuit(
    game.seats.map((s) => ({
      position: s.position,
      alive: s.alive,
      roleId: roleCetteNuit(s),
      loverGroup: s.loverGroup,
    })),
    nuit,
    etat,
  );
  const vol = denouement.vol;
  const volManque = game.thiefVariant === "echange" && nuit.voleurEchange !== undefined && !vol;
  const carteVoleur = voleurPrend;
        ...
        {(denouement.sauves.length > 0 ||
          ...
          <>
            ...
            <ul className="mt-1 flex flex-col gap-1 text-[11px] text-muted-foreground">
              ...
              {vol && (
                <li>
                  Le Voleur prend la carte de {nom(vol.cible)}, qui reçoit la sienne.
                  {vol.cibleMeurt &&
                    ` ${nom(vol.cible)} meurt cette nuit : on retournera la carte du Voleur.`}
                  {vol.voleurMeurt &&
                    ` Le Voleur meurt cette nuit : on retournera la carte qu'il vient de prendre.`}
                  {!vol.cibleMeurt &&
                    !vol.voleurMeurt &&
                    " Chacun découvrira sa nouvelle carte à son réveil."}
                </li>
              )}
              {carteVoleur !== undefined && (
                <li>
                  Le Voleur garde la carte {ROLES_BY_ID[carteVoleur]?.name} ; la sienne retourne au
                  centre.
                </li>
              )}
            </ul>
