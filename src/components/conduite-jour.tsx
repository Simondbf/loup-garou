...
interface EtapeJour {
  ...
  /**
   * Ce que la validation change en plus des morts, et qu'il faudra défaire
   * si le MJ revient sur l'étape. Les morts, elles, se défont toujours : le
   * repère est pris à chaque validation.
   */
  annulation?: Omit<Annulation, "repere">;
...
export function ConduiteJour({
  ...
  const valider = async () => {
    if (!etape) return;
    // Le repère est pris avant d'agir : tout ce qui tombera ensuite sera à
    // relever si le MJ revient sur cette étape.
    const repere = Math.max(0, ...game.seats.map((s) => s.deathOrder));
    if (etape.onValider) await etape.onValider();
    await onJour({
      faites: [...faites, etape.id],
      annulations: {
        ...(game.jour?.annulations ?? {}),
        [etape.id]: { repere, ...(etape.annulation ?? {}) },
      },
    });
  };
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <p className="text-center text-[11px] text-muted-foreground">
        {" "}
        Revenir en arrière défait aussi ce que l'étape avait provoqué : une mort et ses suites, une
        écharpe transmise, un second vote accordé.
      </p>
    </div>
  );
}
...
function construire(game: GameDTO, a: ActionsJour): EtapeJour[] {
  ...
  function declencheurs(position: number): EtapeJour[] {
    ...
    // Le Bouc Émissaire tombé sur une égalité est mort *parce qu'il est* le
    // Bouc : tout le village connaît sa carte avant qu'on la retourne, il n'y
    // a plus rien à prendre en cachette. Mort d'une autre façon — de chagrin,
    // par exemple —, son identité reste secrète et la Servante peut la
    // reprendre comme n'importe quelle autre.
    const boucSacrifie =
      carte?.id === "bouc-emissaire" &&
      (jour.votes ?? []).some((v) => v && v.designe === 0 && v.tranche === position);
    const offreServante =
      servanteDispo && servante && servante.position !== position && !boucSacrifie;
    liste.push({
      id: `carte-${position}`,
    ...
    if (s.isCaptain && etat.avecCapitaine !== false) {
      ...
      liste.push({
        ...
        // L'écharpe revient au Capitaine mort : c'est elle qui fait exister
        // cette étape, et il doit pouvoir la transmettre à quelqu'un d'autre.
        annulation: { capitaine: position },
        rendu: () => (
          <Profils
            joueurs={vivants}
  ...
  // La question reste dans le fil de la première journée une fois
  // répondue : c'est ce qui permet au MJ d'y revenir s'il s'est trompé.
  if (
    game.night <= 1 &&
    (etat.avecCapitaine === undefined || etat.capitaineChoisiLeJour === true) &&
    !game.vainqueur
  ) {
    const reponse = choix["capitaine-option"];
    e.push({
      id: "capitaine-option",
      role: R("capitaine"),
      ...
      aide: "La réponse vaut pour toute la partie. En cas d'erreur, revenez simplement sur cette étape.",
      pret: reponse !== undefined,
      onValider: () => a.onEtat({ avecCapitaine: reponse === 1, capitaineChoisiLeJour: true }),
      annulation: { capitaineOption: true },
      rendu: () => (
        <div className="flex flex-col gap-2">
          <GrosBouton onClick={() => void poser("capitaine-option", 1)} actif={reponse === 1}>
            Oui, le village élit un Capitaine
          </GrosBouton>
          <GrosBouton onClick={() => void poser("capitaine-option", 0)} actif={reponse === 0}>
            Non, on joue sans
          </GrosBouton>
        </div>
      ),
  ...
  // Il reste proposé le jour même où il a parlé, pour qu'on puisse revenir
  // sur sa réponse ; il ne disparaît que les jours suivants.
  const jugeDispo =
    juge !== undefined &&
    (!(etat.pouvoirsUtilises ?? []).includes("juge-begue") || jour.secondTour !== undefined);
  const tours = jour.secondTour ? 2 : 1;

  /** Un Idiot du Village encore dans l'ombre, qu'un vote gracierait. */
  const idiotGraciable = (p: number) => {
    const s = siege(p);
    return !!s && s.alive && s.roleId === "idiot-du-village" && !s.statuses.includes("sans-vote");
  };
  ...
  for (let t = 1; t <= tours; t++) {
    ...
    e.push({
      ...
      // L'Idiot désigné n'est pas tué mais gracié : aucune mort à relever,
      // il faut lui rendre explicitement sa carte secrète et son vote.
      ...(v && v.designe > 0 && idiotGraciable(v.designe)
        ? { annulation: { grace: v.designe } }
        : {}),
      rendu: () => (
        <>
          <Profils
    ...
    if (v && v.designe === 0) {
      ...
      e.push({
        ...
        ...(tranche && tranche > 0 && idiotGraciable(tranche)
          ? { annulation: { grace: tranche } }
          : {}),
        rendu: () =>
          bouc ? (
            <>
