...
import { cartesAttendues, compositionAuto } from "@/data/composition";
...
export interface HostState {
  ...
  angeGagne?: boolean;
  /** La question du Capitaine a été tranchée pendant la journée, et non à la composition. */
  capitaineChoisiLeJour?: boolean;
  /**
...
/**
 * Ce qu'une étape du jour a provoqué, de quoi le défaire.
 *
 * Revenir en arrière ne doit pas seulement rouvrir l'écran : il doit
 * défaire ce que l'étape a fait. Sans quoi un vote repris tue une seconde
 * personne sans ressusciter la première, et le Juge Bègue, une fois son
 * pouvoir consommé, disparaît du fil sans qu'on puisse revenir sur sa
 * réponse.
 */
export interface Annulation {
  /** Rang de mort atteint juste avant l'étape : tout ce qui est tombé après revit. */
  repere: number;
  /** L'Idiot gracié par ce vote : on lui rend sa carte secrète et son vote. */
  grace?: number;
  /** À qui rendre l'écharpe. `null` : à personne. */
  capitaine?: number | null;
  /** Le Juge Bègue retrouve son second vote. */
  juge?: boolean;
  /** Le Loup contaminé par le Chevalier ne l'est plus. */
  gangrene?: boolean;
  /** L'Ange n'a pas encore gagné. */
  ange?: boolean;
  /** La question du Capitaine se repose. */
  capitaineOption?: boolean;
}
...
export interface JourEnCours {
  ...
  secondTour?: boolean;
  /** Pour chaque étape validée, de quoi la défaire si le MJ revient dessus. */
  annulations?: Record<string, Annulation>;
}
...
export const createGame = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const game = await db.pb.creer("games", {
      ...
      phase: "lobby",
      night: 0,
      // Un seul téléphone : le Voleur choisit parmi deux cartes au centre.
      // Plusieurs téléphones : il vole la carte d'un joueur en fin de nuit.
      thief_variant: single ? "centre" : "echange",
...
export const validerProfils = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // En validant les profils, le MJ découvre l'effectif réel : la
    // composition conseillée pour ce nombre l'attend déjà. On ne touche pas à
    // une composition qui tombe juste — il l'a peut-être retouchée avant de
    // revenir aux profils.
    const variante = (game["thief_variant"] as string) || "echange";
    const actuelle = (game["selection"] ?? {}) as Record<string, number>;
    const posees = Object.values(actuelle).reduce((a, b) => a + b, 0);
    const juste = posees === cartesAttendues(seats.length, actuelle, variante);
    await db.pb.modifier("games", game["id"], {
      status: data.valide ? "composition" : "lobby",
      player_count: seats.length,
      ...(data.valide && !juste
        ? { selection: compositionAuto(seats.length, !!game["single_device"], variante) }
        : {}),
    });
    const fresh = await loadGame(db, data.code);
    return buildDTO(db, fresh, data.token);
...
/**
 * Revenir sur la dernière étape validée du jour, en défaisant ce qu'elle a fait.
 *
 * Tout se passe ici, en une seule opération : les morts provoquées depuis
 * l'étape reviennent en jeu — cascade des Amoureux comprise, puisqu'on
 * rappelle tout ce qui est tombé après le repère —, leur carte redevient
 * secrète, l'écharpe retourne à qui la portait, et les pouvoirs consommés
 * par l'étape sont rendus.
 */
export const reculerJour = createServerFn({ method: "POST" })
  .inputValidator((d: { code: string; token: string }) => d)
  .handler(async ({ data }) => {
    const db = await base();
    const game = await requireHost(db, data.code, data.token);
    const jour = { ...((game["jour"] ?? {}) as JourEnCours) };
    const faites = [...(jour.faites ?? [])];
    const derniere = faites.pop();
    if (derniere === undefined) {
      const fresh = await loadGame(db, data.code);
      return buildDTO(db, fresh, data.token);
    }
    const annulations = { ...(jour.annulations ?? {}) };
    const a = annulations[derniere];
    delete annulations[derniere];

    const etat = { ...((game["host_state"] ?? {}) as HostState) };
    if (a) {
      const seats = await seatsDe(db, game["id"]);
      for (const place of seats) {
        const tombe = !place["alive"] && ((place["death_order"] as number) ?? 0) > a.repere;
        if (tombe) {
          await db.pb.modifier("seats", place["id"], {
            alive: true,
            death_cause: "",
            death_phase: "",
            death_order: 0,
            // Retournée à sa mort, la carte redevient secrète — sauf pour le
            // Villageois-Villageois, dont la carte est publique d'office.
            public_role: place["role_id"] === "villageois-villageois",
          });
          if (place["role_id"] === "ancien") etat.villageSansPouvoirs = false;
        }
        if (a.grace === place["position"]) {
          const statuts = ((place["statuses"] ?? []) as string[]).filter((x) => x !== "sans-vote");
          await db.pb.modifier("seats", place["id"], { statuses: statuts, public_role: false });
        }
      }
      if (a.capitaine !== undefined) {
        for (const place of seats) {
          const doit = place["position"] === a.capitaine;
          if (!!place["is_captain"] !== doit) {
            await db.pb.modifier("seats", place["id"], { is_captain: doit });
          }
        }
      }
      if (a.juge) {
        etat.pouvoirsUtilises = (etat.pouvoirsUtilises ?? []).filter((x) => x !== "juge-begue");
        delete jour.secondTour;
      }
      if (a.gangrene) etat.gangrene = 0;
      if (a.ange) etat.angeGagne = false;
      if (a.capitaineOption) {
        delete etat.avecCapitaine;
        etat.capitaineChoisiLeJour = false;
      }
    }

    jour.faites = faites;
    jour.annulations = annulations;
    await db.pb.modifier("games", game["id"], { jour, host_state: etat });
    const fresh = await loadGame(db, data.code);
    return buildDTO(db, fresh, data.token);
  });
...
export interface DenouementNuit {
  ...
  patchEtat: HostState;
  /**
   * L'échange du Voleur, appliqué au lever du jour. Il a lieu même si l'un
   * des deux meurt cette nuit : celui qui tombe tombe avec la carte qu'il
   * tient au matin.
   */
  vol?: { voleur: number; cible: number; cibleMeurt: boolean; voleurMeurt: boolean };
}
...
export function denouementNuit(
  ...
  // Le Voleur, s'il y en a un qui vole cette nuit. Il agit en toute fin de
  // nuit, comme tout le monde avant le lever du jour : même désigné par les
  // Loups, il ne le sait pas encore, et il vole.
  const voleur =
    nuit.voleurEchange !== undefined
      ? places.find((s) => s.roleId === "voleur" && s.alive)
      : undefined;
  const cible = nuit.voleurEchange;
  ...
  /*
   * Les cartes changent de main, les morts restent aux personnes.
   *
   * Si la cible était désignée cette nuit, elle meurt quand même — mais sous
   * la carte du Voleur, qu'on retournera au matin. Le Voleur, lui, survit
   * avec le rôle qu'il vient de prendre. Et s'il était lui-même désigné, il
   * tombe avec la carte volée, pendant que sa cible reste en vie sous la
   * carte du Voleur.
   */
  if (voleur && cible !== undefined && cible !== voleur.position) {
    resultat.vol = {
      voleur: voleur.position,
      cible,
      cibleMeurt: dejaMort.has(cible),
      voleurMeurt: dejaMort.has(voleur.position),
    };
  }
  return resultat;
}
...
export const resolveNight = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Variante « cartes au centre » : le Voleur joue dès la première nuit le
    // rôle qu'il a pris. S'il a pris l'Ancien, il encaisse la morsure comme
    // lui — la résolution doit donc le voir sous ce rôle-là.
    const premiereNuit = ((game["night"] as number) ?? 1) <= 1;
    const voleurCentre =
      premiereNuit && game["thief_variant"] === "centre" ? nuit.voleurCarte : undefined;

    const { morts, sauves, infecte, enfantTransforme, patchEtat, vol } = denouementNuit(
      seats.map((s) => ({
        position: s["position"] as number,
        alive: !!s["alive"],
        roleId:
          voleurCentre && s["role_id"] === "voleur"
            ? voleurCentre
            : (s["role_id"] as string) || null,
    ...
    /*
     * Le Voleur, en dernier.
     *
     * Rien n'a changé de mains pendant la nuit : le Maître du Jeu a pu
     * revenir sur ses étapes sans que les cartes aient déjà bougé, et c'est
     * ici seulement que le vol prend effet.
     */
    if (voleurCentre) {
      // Il garde la carte prise au centre ; la sienne y retourne.
      const voleur = seats.find((s) => s["role_id"] === "voleur" && s["alive"]);
      const centre = [...((game["center_cards"] ?? []) as string[])];
      const i = centre.indexOf(voleurCentre);
      if (voleur && i !== -1) {
        centre[i] = "voleur";
        await db.pb.modifier("seats", voleur["id"], { role_id: voleurCentre, seen: false });
        await db.pb.modifier("games", game["id"], { center_cards: centre });
      }
    } else if (vol) {
      // L'échange a lieu même si l'un des deux meurt cette nuit : chacun tombe
      // avec la carte qu'il tient au matin, et c'est elle qu'on retournera.
      const voleur = parPosition(vol.voleur);
      const cible = parPosition(vol.cible);
      if (voleur && cible) {
        const carteVolee = (cible["role_id"] as string) || "";
        await db.pb.modifier("seats", voleur["id"], { role_id: carteVolee, seen: false });
        await db.pb.modifier("seats", cible["id"], { role_id: "voleur", seen: false });

        /*
         * La transformation suit la carte, l'infection suit la personne.
         *
         * Un Enfant Sauvage dont le modèle est mort, un Chien-Loup qui a
         * choisi la meute : c'est leur carte qui fait d'eux des Loups. Le
         * Voleur qui la prend en hérite, et celui qu'il dépouille redevient un
         * simple Voleur. Une victime de l'Infect Père, elle, reste Loup quelle
         * que soit la carte qu'on lui laisse : c'est elle qui a été mordue.
         */
        const devenus = new Set(patchEtat.devenusLoups ?? etat.devenusLoups ?? []);
        const modele = patchEtat.modele ?? nuit.modele ?? etat.modele;
        const modeleMort = modele !== undefined && !parPosition(modele)?.["alive"];
        const choixChien = patchEtat.chienLoup ?? etat.chienLoup;
        const loupParSaCarte =
          devenus.has(vol.cible) &&
          ((carteVolee === "enfant-sauvage" && modeleMort) ||
            (carteVolee === "chien-loup" && choixChien === "loups"));
        if (loupParSaCarte) {
          devenus.delete(vol.cible);
          devenus.add(vol.voleur);
          patchEtat.devenusLoups = [...devenus];
        }
      }
    }
