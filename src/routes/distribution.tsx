    ...
    <main className="mx-auto min-h-screen w-full max-w-md px-5 pt-10 pb-16">
          ...
          {(() => {
            ...
            return (
              <div className="surface p-4">
                ...
                {/* Une clé par place : le champ repart vide pour chaque
                    joueur, sans effet pour le vider à la main. */}
                <SaisieProfil
                  key={aRemplir.position}
                  onValider={(nom) => nommer(aRemplir.position, nom)}
                />
      ...
      {seat && revele && seat.roleId && (
        <CarteRevelee
          seat={seat}
          etat={
            game.singleDevice ? undefined : game.mesEtats.find((x) => x.position === seat.position)
          }
          onClose={() => {
            setActive(null);
            setRevele(false);
 ...
 */
/**
 * La saisie d'un profil, validée par un bouton et rien d'autre.
 *
 * Un profil se verrouille à la première validation. Un enregistrement
 * automatique après une courte pause — comme dans les autres champs de
 * prénom — validait donc « t » dès qu'un joueur hésitait sur la lettre
 * suivante, et la suite de sa saisie tombait dans le profil de son voisin.
 * Ici, rien ne part tant qu'on n'a pas appuyé sur Valider ou sur Entrée.
 */
function SaisieProfil({ onValider }: { onValider: (nom: string) => Promise<void> | void }) {
  ...
  return (
    <div className="mt-3 flex flex-col gap-2">
      <input
        value={nom}
        onChange={(e) => setNom(e.target.value.slice(0, 24))}
        onKeyDown={(e) => {
          if (e.key === "Enter") void valider();
        }}
        placeholder="Prénom"
        autoComplete="off"
        className="w-full rounded-xl border border-border bg-secondary px-3 py-3 text-base outline-none focus:border-primary"
      />
      <Button className="w-full" disabled={!propre || envoi} onClick={() => void valider()}>
        {envoi ? "Validation…" : "Valider mon profil"}
      </Button>
    </div>
  );
}
...
/**
 * La carte d'un joueur, et ce qu'il sait de sa propre situation.
 *
 * L'état — potions restantes, cible interdite, camp choisi, modèle — dit
 * presque toujours quel rôle on tient. Il ne s'affiche donc qu'ici, avec la
 * carte, que le joueur ouvre à l'abri des regards : sur un téléphone
 * partagé à deux ou trois, le laisser dans la liste des places revenait à
 * donner son rôle à ses voisins.
 */
function CarteRevelee({
  ...
  return (
    <div className="animate-flip-in flex flex-col gap-4">
      ...
      {etat && <MonEtat seat={seat} etat={etat} />}
