...
function Accueil() {
  ...
  // Le MJ ne reprend qu'une partie qui se joue, ou un salon où des joueurs
  // l'attendent déjà sur leur téléphone. Le salon vide qui s'ouvre tout seul
  // après une fin de partie n'est pas une partie en cours : pour en lancer
  // une nouvelle, on passe par « Créer une partie ».
  const enJeu = game?.status === "dealt";
  const salonHabite =
    (game?.status === "lobby" || game?.status === "composition") &&
    !game.singleDevice &&
    game.seats.length > 0;
  const reprise =
    hydrated && session && game && (session.host ? enJeu || salonHabite : game.status !== "ended");
