...
import { ChoixCartesComedien, ChoixRoles } from "@/components/choix-roles";
import {
  ajusterRole,
  cartesAttendues,
  compositionAuto,
...
function NouvellePartie() {
  ...
  // La variante du Voleur découle du mode : deux cartes au centre sur un seul
  // téléphone, vol de la carte d'un joueur en fin de nuit sur plusieurs.
  const varianteVoleur: "centre" | "echange" = singleDevice ? "centre" : "echange";
  ...
  // Sans ses trois cartes, le Comédien bloquerait la distribution : on ne
  // laisse pas créer la partie avant qu'elles soient choisies.
  const comedienPret =
    !selection["comedien"] || cartesComedien.filter((id) => !selection[id]).length === 3;
    ...
    try {
      const { code } = await createGame({
        data: {
          ...
          singleDevice,
          // Une carte devenue distribuée depuis son choix ne peut plus aller au
          // centre : on la retire plutôt que de laisser le serveur refuser.
          comedienCartes: selection["comedien"]
            ? cartesComedien.filter((id) => !selection[id])
            : [],
        },
      });
      ...
      {step === 2 && (
        <section>
          ...
          <ChoixRoles
            pioche={pioche}
            selection={selection}
            attendu={(sel) => cartesAttendues(count, sel, varianteVoleur)}
            onSelection={setSelection}
            onDetail={setDetail}
          />

          {selection["voleur"] && singleDevice ? (
            <p className="mb-3 rounded-xl border border-border p-3 text-[11px] text-muted-foreground">
              Le Voleur choisit la première nuit parmi deux cartes posées au centre, face cachée :
              deux cartes de plus sont à prévoir.
            </p>
          ) : null}
          ...
          {selection["comedien"] ? (
            <ChoixCartesComedien
              selection={selection}
              cartes={cartesComedien.filter((id) => !selection[id])}
              onCartes={setCartesComedien}
            />
          ) : null}

          <Button
            className="w-full py-4"
            disabled={total !== cible || busy || !comedienPret}
            onClick={() => void lancer()}
          >
            {total === cible
              ? busy
                ? "Création…"
                : !comedienPret
                  ? "Choisissez les 3 cartes du Comédien"
                  : "Créer la partie"
              : total < cible
