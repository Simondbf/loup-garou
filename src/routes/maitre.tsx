...
function Maitre() {
  ...
  const distributionPossible =
    ...
    (!game.selection["comedien"] || game.comedienCartes.length === 3);
          ...
          <div className={cn("mb-4 flex gap-2", !game.singleDevice && "hidden")}>
            {(
              ...
              <button
                key={id}
                onClick={() => {
                  // Quitter l'onglet cache la carte montrée : en revenant, elle
                  // ne doit pas réapparaître sous les yeux du joueur suivant.
                  setOnglet(id);
                  setRevealFrom(null);
                }}
                className={cn(
            ...
            <section className="flex flex-col gap-3">
              ...
              {game.phase === "jour" && (
                <ConduiteJour
                  ...
                  onReculer={() => run(reculerJour({ data: { code: game.code, token } }))}
                  onTerminer={async () => {
    ...
    <div className="surface p-4">
      ...
      <div className="mt-4">
        <ChoixRoles
          pioche={pioche}
          selection={selection}
          attendu={(sel) => cartesAttendues(effectif, sel, variante)}
