...
Le tableau `compositions-conseillees.xlsx`, à la racine, fait foi : une ligne
par nombre de joueurs (7 à 30), une colonne par rôle. `compositionAuto` en
reprend les lignes telles quelles dans `src/data/composition.ts`. Pour changer
une composition, on modifie le tableau, puis on reporte ses lignes dans le code.

Version provisoire : de 14 à 30 joueurs, les lignes comptent plus de cartes
que de joueurs, et le MJ retire l'excédent avant de distribuer.

## Le Voleur
La variante découle du mode, sans choix possible. **Plusieurs téléphones** :
il échange sa carte avec celle d'un joueur en toute fin de nuit, chaque nuit.
**Un seul téléphone** : deux cartes au centre, il y choisit la première nuit,
**en tout premier**, et
joue le rôle pris dès cette nuit-là (`roleCetteNuit` dans la conduite, et le
même calcul côté serveur pour la résolution).

Dans les deux cas, rien ne change de mains pendant la nuit : le vol n'est
appliqué qu'au lever du jour, dans `resolveNight`. Le MJ peut revenir en
arrière sans que les cartes aient déjà bougé.

Les cartes changent de main, les morts restent aux personnes. S'il vole un
joueur désigné cette nuit, ce joueur meurt quand même — mais sous la carte du
Voleur, qu'on retournera au matin — et le Voleur survit avec le rôle volé. S'il
était lui-même désigné, il tombe avec la carte qu'il vient de prendre.

La transformation suit la carte, l'infection suit la personne : un Enfant
Sauvage dont le modèle est mort, ou un Chien-Loup qui a choisi la meute,
transmet son camp au Voleur qui prend sa carte ; une victime de l'Infect Père
reste Loup quelle que soit la carte qu'on lui laisse.
...
## Revenir en arrière dans la journée
« Précédent » ne se contente pas de rouvrir l'écran : il défait ce que l'étape
avait provoqué, côté serveur, en une seule opération (`reculerJour`). À chaque
validation, le moteur note un repère — le rang de mort atteint juste avant — et
ce que l'étape change en plus (`Annulation`). Reculer relève tout ce qui est
tombé après le repère, cascade des Amoureux comprise, remet leur carte au
secret, rend l'écharpe à qui la portait, rend au Juge Bègue son second vote,
et repose la question du Capitaine.

Sans cela, un vote repris tuait une seconde personne sans relever la première,
et un second vote pouvait se rejouer indéfiniment.

Règle pour toute nouvelle étape : une décision se note au toucher et ne
s'applique qu'à la validation. Une étape qui disparaît du fil dès qu'on
touche un bouton ne peut plus être validée ni retrouvée.
